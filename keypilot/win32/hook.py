...
from keypilot.win32.structs import (
    HOOKPROC,
    KBDLLHOOKSTRUCT,
    MSLLHOOKSTRUCT,
    kernel32,
...
class HookThread:
        ...
        try:
            ...
            if direction is not None:
                event = KeyEvent(
                    ...
                    ours=kb.dwExtraInfo == C.KEYPILOT_SIGNATURE,
                    time_ms=kb.time,
                    t=t0,
                )
    ...
    def _on_mouse(self, ncode: int, wparam: int, lparam: int) -> int:
        ...
        try:
            ...
            event = MouseEvent(
                ...
                ours=ms.dwExtraInfo == C.KEYPILOT_SIGNATURE,
                time_ms=ms.time,
                t=t0,
            )
    ...
    def start(self, timeout: float = 5.0) -> None:
        self._ready.clear()
        self._error = None
        self._thread = threading.Thread(target=self._run, name="keypilot-hook", daemon=True)
        self._thread.start()
